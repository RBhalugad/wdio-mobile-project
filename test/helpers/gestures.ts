...
const move  = (x: number, y: number, duration = 0) => ({ type: 'pointerMove' as const, duration, x, y });
...
class Gestures {
  ...
  async tapByCoordinates(x: number, y: number): Promise<void> {
    await driver.performActions([
      touch('finger1', [move(x, y), down(), pause(80), up()]),
    ]);
  ...
  async swipeElementLeft(element: WebdriverIO.Element): Promise<void> {
    ...
    await driver.performActions([
      touch('finger1', [
        ...
        down(),
        pause(300),
        { type: 'pointerMove' as const, duration: 600, x: Math.round(x + width * 0.1), y: Math.round(y + height / 2) },
        up(),
      ]),
    ]);
    await driver.releaseActions();
  ...
  private async _swipe(direction: SwipeDirection, scrollPercent: number): Promise<void> {
    ...
    let fromX = cx, toX = cx, fromY = cy, toY = cy;
    switch (direction) {
      case 'up':
  ...
  private async _pinch(centerX: number, centerY: number, spread = true): Promise<void> {
    ...
    await driver.performActions([
      touch('finger1', [
        ...
        down(),
        { type: 'pointerMove' as const, duration: 600, x: centerX - 60 - offset, y: centerY },
        up(),
      ]),
      touch('finger2', [
        move(centerX + 60, centerY),
