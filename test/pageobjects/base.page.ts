...
class BasePage {
  ...
  async swipeScreen(direction: SwipeDirection, scrollPercent = 0.5): Promise<void> {
    ...
    const vectors: Record<SwipeDirection, Partial<{ fromY: number; toY: number; fromX: number; toX: number }>> = {
      up:    { fromY: Math.round(height * 0.75), toY: Math.round(height * (0.75 - scrollPercent)) },
      down:  { fromY: Math.round(height * 0.25), toY: Math.round(height * (0.25 + scrollPercent)) },
      left:  { fromX: Math.round(width * 0.8),   toX: Math.round(width * (0.8 - scrollPercent)) },
      right: { fromX: Math.round(width * 0.2),   toX: Math.round(width * (0.2 + scrollPercent)) },
    };
